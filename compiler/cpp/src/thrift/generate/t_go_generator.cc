...
void t_go_generator::generate_process_function(t_service* tservice, t_function* tfunction) {
  ...
  f_types_ << "= p.interceptor(ctx, args, func(ctx context.Context, arg interface{}) (result interface{}, err error) {" << endl;
  ...
  f_types_ << indent() << "})" << endl;
    ...
    if (!tfunction->is_oneway()) {
        if (!tfunction->get_returntype()->is_void()) {
            f_types_ << "if res != nil {" << endl;
            f_types_ << indent() << "retval = res.(" << type_to_go_type(tfunction->get_returntype()) << ")" << endl;
            f_types_ << "}" << endl;
        }
    }
